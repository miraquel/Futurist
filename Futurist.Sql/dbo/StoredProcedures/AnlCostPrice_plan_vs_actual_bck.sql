...
-- 4200031, 92530721
-- =============================================
CREATE PROCEDURE [dbo].[AnlCostPrice_plan_vs_actual_bck]
@Room int = 1,
@VerId int = 72,	
@Year int = 2025,
@Month int = 8,
...
BEGIN
	...
	DECLARE @CostDetail table (
		...
		,ReferenceId nvarchar(20)
		,Qty numeric(32,16)
		,QtyInKg numeric(32,16)
		,CostAmount numeric(32,16)
	) 
	...
	INSERT INTO @Por (ITEMID,INVENTBATCHID,PRODID,SALESQTY)
		SELECT a.ITEMID, a.INVENTBATCHID, b.PRODID, SUM(a.QTY) as [QTY]	
		FROM [SCMBI].[dbo].[FactSalesInvoice] a
		JOIN [AXGMKDW].[dbo].[FactPor] b ON b.ITEMID = a.ITEMID AND b.INVENTBATCHID = a.INVENTBATCHID
		WHERE a.ITEMID = @ItemId
			AND YEAR(INVOICEDATE) = @Year
			AND MONTH(INVOICEDATE) = @Month
		GROUP BY a.ITEMID, a.INVENTBATCHID, b.PRODID

	INSERT INTO @CostDetail (ProdId,EndedDate,ProductId,InventBatchId,Level_,ItemId,ItemName,InventItemType
		,InventCostType,ReferenceCategory,ReferenceId,Qty,QtyInKg,CostAmount)
			SELECT a.[PRODID]
				  ,a.[ENDEDDATE]
				  ,a.[PRODUCTID]
				  ,a.[INVENTBATCHID]
				  ...
				  ,a.[REFERENCECATEGORY]
				  ,a.[REFERENCEID]
				  ,-1*a.[QTY] * b.[SALESQTY] as [QTY]
				  ,-1*a.[QTY] * i.NETWEIGHT / 1000 * b.[SALESQTY] as [QTYINKG]
				  ,-1*a.[COSTAMOUNT] * b.[SALESQTY] as [COSTAMOUNT]
			  FROM [AXGMKDW].[dbo].[FactGmk_RmpmCalcTable] a
			  JOIN [AXGMKDW].[dbo].[DimItem] i ON i.ITEMID = a.ITEMID
			  JOIN @Por b ON b.PRODID = a.PRODID

	INSERT INTO @Result ([Year],[Month],InventItemType,ItemId,ItemName,Qty,QtyInKg,ActPrice)
		SELECT @Year,@Month,InventItemType, ItemId, ItemName
			,SUM(Qty) as [Qty]
			,SUM(QtyInKg) as [QtyInKg]
			,SUM(CostAmount) / SUM(Qty) as [Price]
			FROM @CostDetail
			WHERE InventItemType in (0,1,3)
			GROUP BY InventItemType, ItemId, ItemName
			ORDER BY ItemId

	DECLARE @Tot numeric(32,16)
	SELECT @Tot = SUM(Qty * ActPrice) FROM @Result WHERE InventItemType in (0,1)

	UPDATE @Result
		SET Contr = (Qty * ActPrice) / @Tot 	
		WHERE InventItemType in (0,1)
