...
-- =============================================
CREATE PROCEDURE [dbo].[AnlFgPrice_plan_vs_actual_new]
@Room int = 1,
@VerId int = 66,	--68
@Year int = 2025,
@Month int = 6		--7
...
BEGIN
	DECLARE @Result table (
		...
		,[ActYield] numeric(32,16)
		,[ActYieldLm] numeric(32,16)
	) 
	...
	SELECT 
		...
		,MAX(y.[YIELD]) as [YIELD]
		,MAX(ylm.[YIELD]) as [YIELD]
	...
	LEFT JOIN (
		SELECT f.ITEMID, SUM(f.QTY*f.YIELD) / NULLIF(SUM(f.QTY),0) as [YIELD]
		FROM (
				SELECT a.[ITEMID]
					,a.[INVENTBATCHID] 
					,SUM(a.[QTY]) as [QTY]
					,MAX(b.RAFQTY)/NULLIF(MAX(b.RMQTY),0) *-1 as [YIELD]
				  FROM [AXGMKDW].[dbo].[FactSalesInvoice] a
				  JOIN [AXGMKDW].[dbo].[FactPor] b ON b.ITEMID = a.ITEMID AND b.INVENTBATCHID = a.INVENTBATCHID
				  WHERE YEAR(a.INVOICEDATE) = @Year
						AND MONTH(a.INVOICEDATE) = @Month
						--AND a.ITEMID = '4200031'
					GROUP BY a.ITEMID,a.[INVENTBATCHID] 
			) f
			GROUP BY f.ITEMID
		)y ON y.[ITEMID] = a.[ITEMID]
	...
	LEFT JOIN (
		...
		FROM (
				SELECT a.[ITEMID]
				  ...
				  FROM [AXGMKDW].[dbo].[FactSalesInvoice] a
				  JOIN [AXGMKDW].[dbo].[FactPor] b ON b.ITEMID = a.ITEMID AND b.INVENTBATCHID = a.INVENTBATCHID
				  WHERE YEAR(a.INVOICEDATE) = YEAR(DATEADD(DAY,-1, DATEFROMPARTS(@Year,@Month,1)))
						AND MONTH(a.INVOICEDATE) = MONTH(DATEADD(DAY,-1, DATEFROMPARTS(@Year,@Month,1)))
						--AND a.ITEMID = '4200031'
					GROUP BY a.ITEMID,a.[INVENTBATCHID] 
			) f
			GROUP BY f.ITEMID
		)ylm ON ylm.[ITEMID] = a.[ITEMID]
	...
	SELECT Room,VerId,[Year],[Month],ItemId,ItemName,UnitId
		...
		,ActRmPrice,ActPmPrice,ActStdCostPrice
		,ActStdCostPriceLm
		,ActCostValue/NULLIF(@ActCostValueTot,0) as [Cont]
		,ActCostPrice/NULLIF(PlanCostPrice,0) as [A/P]
