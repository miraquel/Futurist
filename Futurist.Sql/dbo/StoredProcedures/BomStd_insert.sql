...
-- EXEC [BomStd_insert] 99
-- Durasi : 2 menit, after index 3 detik
-- =============================================
CREATE PROCEDURE [dbo].[BomStd_insert]
	@Room int = 99
...
----**PROSES LEVEL PERTAMA (1)
	...
	SELECT @Room,[BOMID],[PRODUCTID],[ITEMID],[ITEMNAME]
		...
		,[LINETYPE],[SUBBOMID],[BomId],@Level,[FromDate],[ToDate],@CreatedBy,GETDATE()
	  FROM AXGMKDW.dbo.[FactBom]
	  WHERE ACTIVE = 1
		AND FROMQTY = 1
		--AND FROMDATE <= GETDATE() 
		AND ( [TODATE] = '1900-01-01' OR [TODATE] >= CONVERT(DATE, GETDATE())  )
