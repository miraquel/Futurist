...
-- EXEC [PsBomEntry_BomStdInsert]] 1
-- Durasi : 2 menit, after index 3 detik
-- =============================================
CREATE PROCEDURE [dbo].[PsBomEntry_BomStdInsert_new]
	@Room int = 1
...
BEGIN
	...
	DELETE FROM PsBomEntry_BomStd WHERE Room = @Room
	
----**PROSES LEVEL PERTAMA (1)
	SET @Level = 1

	INSERT INTO PsBomEntry_BomStd ([Room],[BomId],[ProductId],[ItemId],[ItemName],[BomQty],[BomQtySerie]
      ,[LineType],[SubBomId],[Ref],[Level],[FromDate],[ToDate],CreatedBy,CreatedDate)
	SELECT @Room,a.[BOMID],a.[PRODUCTID],a.[ITEMID],a.[ITEMNAME]
		,a.[BOMQTY]/a.[BOMQTYSERIE]
		,a.[BOMQTYSERIE]/a.[BOMQTYSERIE]
...
----**PROSES LEVEL KEDUA DST (2)
    ...
    BEGIN
		...
		BEGIN		
			...
			DELETE FROM PsBomEntry_BomStd WHERE RecId = @RecId	
		END	
	...
	IF EXISTS(
		...
		LEFT JOIN PsBomEntry_BomStd b ON b.ProductId = a.ItemId
			AND b.BomId = a.BomIdEntry
		JOIN AXGMKDW.dbo.DimItem i ON i.ITEMID = a.ItemId
		WHERE a.Room = @Room
			AND b.BomId is null
		UNION 
		--2. Cek PsBomEntry_BomStd belum breakdown
		SELECT a.[Room]
