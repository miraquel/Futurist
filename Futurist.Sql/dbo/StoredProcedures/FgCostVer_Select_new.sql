...
-- EXEC [FgCostVer_Select_new] 63,1
-- durasi 0 detik utk 4125 rows
-- =============================================
CREATE PROCEDURE [dbo].[FgCostVer_Select_new]
	@Room int = 1,
	@VerId int = 63
AS
BEGIN
	...
	SELECT a.[Room]
		,a.[RecId] as [RofoId]
		  ...
		  ,b.[RmPrice]
		  ,b.[PmPrice]
		  ,b.[RmPrice]/b.[Yield]+b.[PmPrice] as [RmPm+Y]
		  ,b.[StdCostPrice]
		  ,b.[CostPrice] as [CostPrice]
		  ,p.[SalesPriceIndex] as [SalesPrice]
		  ,'=>' as [Previous Calc]
		  ,c.[QtyRofo] as [RofoQty Prev]
		  ,c.[Yield] as [Yield Prev]
		  ,c.[RmPrice] as [Rm Prev]
		  ,c.[PmPrice] as [Pm Prev]
		  ,c.[RmPrice]/c.[Yield]+c.[PmPrice] as [RmPm+Y Prev]
	  ...
	  ORDER BY (b.[QtyRofo]*b.COSTPRICE) DESC
END
