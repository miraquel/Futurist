...
public class MaterialPlanRepository : IMaterialPlanRepository
{
    private readonly IDbConnection _sqlConnection;

    public MaterialPlanRepository(IDbConnection sqlConnection)
    {
        _sqlConnection = sqlConnection;
    }

    public async Task<SpTask?> ProcessMaterialPlanAsync(ProcessMaterialPlanCommand command)
    {
        await _sqlConnection.ExecuteAsync("SET ARITHABORT ON;");
        const string query = "EXEC MaterialPlan_Calc @Room, @VerId, @Year, @Month";
    ...
    public async Task<IEnumerable<int>> GetMaterialPlanRoomIdsAsync(GetMaterialPlanRoomIdsCommand command)
    {
        await _sqlConnection.ExecuteAsync("SET ARITHABORT ON;");
    ...
    public async Task<IEnumerable<int>> GetMaterialPlanVerIdsAsync(GetMaterialPlanVerIdsCommand command)
    {
        await _sqlConnection.ExecuteAsync("SET ARITHABORT ON;");
    ...
    public async Task<IEnumerable<int>> GetMaterialPlanYearsAsync(GetMaterialPlanYearsCommand command)
    {
        await _sqlConnection.ExecuteAsync("SET ARITHABORT ON;");
    ...
    public async Task<IEnumerable<int>> GetMaterialPlanMonthsAsync(GetMaterialPlanMonthsCommand command)
    {
        await _sqlConnection.ExecuteAsync("SET ARITHABORT ON;");
